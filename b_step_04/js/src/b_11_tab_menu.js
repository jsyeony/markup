...
(function($){
  // $.getJSON("../json/menu_data.json", function(data){});
  // step 1 : json data 불러오기
  $.ajax({
    ...
    // title 에 따른 메뉴 구성 : 함수 호출 > tabMenuFn(순서);
    // title 에 따른 메뉴 구성 : 구성 사용 > tabMenuSet;
    // --------------------------------------------------------------------------
    ...
    // tabTitle 내부에 탭 메뉴 제목부 구성 tabTitleCheck를 이용
    tabTitle.html('<ul></ul>');
    ...
    tabBtn.on('click', function(e){
      ...
      // tabMenuFn(n);
      // tabMenuFn을 통해 찾아낸 타입구성을 재설계하여 배치 할 수 있는 함수 호출
      tabMenuSetFn(n);
    });
